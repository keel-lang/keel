...
/// Dispatches a value method (`xs.map`, `s.upper`, …) against `host` — the
/// `Host` seam this runs identically over `Interpreter` and `CompiledHost`
/// (`designs/llvm-compilation.md` §2.7), so a closure-taking or impl-method
/// arm works the same regardless of which concrete `host` reaches it, modulo
/// what that type actually implements. `CompiledHost::call_closure`/
/// `call_task` are still `todo!()`, so only the closure-free, impl-free
/// subset (currently just `Value::String` methods) is safe to reach from
/// compiled code — but nothing in `keel-kir`/`keel-codegen` calls this yet
/// (there is no `keel_rt_call_value_method` FFI entry point and no
/// value-method lowering), so `CompiledHost::call_method_on_value` is only
/// reachable today from `keel-rt-ffi`'s own tests, not from a compiled
/// program. Wiring it up is a later M3 slice, tracked under #114.
pub async fn call_method_on_value(
    host: &mut dyn Host,
    obj: Value,
    method: &str,
    ...
    if let Some(task) = host.find_impl_task(&obj, method) {
        let mut call_args = vec![CallArgValue {
            name: None,
            value: obj,
        }];
        call_args.extend(args);
        return host.call_task(method, &task, call_args).await;
    }
    ...
    match (&obj, method) {
        ...
        (Value::String(s), "pad") => {
            ...
            let pad_char_str =
                optional_named_or_positional_str_arg(&args, "char", 1, "str.pad")?.unwrap_or(" ");
        ...
        (Value::List(items), "min" | "max") => {
            ...
            host.call_namespace_method("__global", method, call_args)
                .await
        }
        ...
        (Value::List(items), "sort") => {
            ...
            let cmp_task = items
                .first()
                .and_then(|first| host.find_impl_task(first, "compare"));
        ...
        (Value::String(s), "format") => {
            let pattern = optional_named_or_positional_str_arg(&args, "as", 0, "datetime.format")?
                .unwrap_or_default();
            match chrono::DateTime::parse_from_rfc3339(s) {
