...
| K-02 | Unify failure model | `T?`, `Result[T,E]`, `try/catch`, `fallback:`, `??`, and `!` overlap. Define one clean rule: nullable for absence, `Result` for fallible ops (HTTP, email, LLM, JSON), thrown errors for programmer faults. AI failures need reasons, not just `none`. | **plan** | Core language coherence. `Ai.classify(...) -> Urgency?` hiding whether the model was unavailable, output was unparseable, or a timeout occurred is a real design bug. |
...
| K-15 | Interface conformance checks | When an `@provider` claims to implement an `interface`, the checker should verify the provider's handlers match. Currently parser-only. | **plan** | Follows from K-14. If interfaces dispatch at runtime, they must be checked at compile time. Otherwise it's just parser scaffolding that trains users to trust something unenforced. |
...
| K-55 | Module system | `use` parses but doesn't resolve. Every `.keel` file is a standalone program. Design exports, visibility, cyclic imports, versioning. | **plan** | Essential for any non-trivial Keel program. Agents will quickly split across files. Modules also affect security: importing a package that adds capabilities must be explicit and auditable. |
