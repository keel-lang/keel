...
| `as T` coercions + `typeof()` | [x] | `as T` now performs real coercions: `int↔float`, `int/float/bool→str`, `str→int/float/bool` (raises on invalid), `none→any` (raises), `dynamic` pass-through. `typeof(x) -> str` added as a prelude free function returning the runtime type name (declared name for structs/enums). |
