...
|---|---|---|---|
| `Ai` | [~] | `classify`, `summarize`, `draft`, `extract`, `translate`, `decide`, `prompt` | `embed` returns `[]`; `Ai.install(provider)` not registered; `classify(considering:)` accepted but not sent to LLM; `decide` returns a plain `{choice, reason, confidence: 1.0}` map instead of a `Decision[T]` type; `prompt(response_format: json)` ignored |
