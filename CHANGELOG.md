...
- **`keel dap`'s recognized commands are now one enum (`DapCommand`) instead of two independently-maintained string-match lists.** `dispatch_running_request` and the paused-request loop each had their own `match` over raw command strings; both matches now go through `DapCommand::parse` and match it exhaustively, so a future new command forces both call sites to decide what it does instead of silently disagreeing. No user-facing behavior change.
...
- **LLVM/`inkwell` toolchain spike for the native backend.** Before any codegen crate is written, a standalone throwaway crate (`spikes/llvm-poc/`, outside the root Cargo workspace so `keel run`/`check`/`lsp` builds stay LLVM-free) proves the full emit → object → link → run loop: build LLVM IR with `inkwell`, emit a native object via `TargetMachine`, link with the system `cc`, and run the result. Verified on both macOS arm64 (native) and Linux x86_64 (Ubuntu 24.04 container). Pins `inkwell` 0.9.0 (`llvm22-1` feature, LLVM 22) and documents both install paths (`brew install llvm@22` on macOS; `apt.llvm.org`'s `llvm.sh` plus `libpolly-22-dev` on Linux, both via `LLVM_SYS_221_PREFIX`) in `designs/llvm-toolchain-spike.md`.
...
- **`keel dap` ignored a client's declared 0-indexed line/column convention.** The server always assumed `linesStartAt1`/`columnsStartAt1` were `true`, so a client that declares 0-indexed lines (uncommon, but part of the DAP spec) saw breakpoint and stack-frame line numbers off by one. `initialize`'s indexing flags are now read and applied both to incoming breakpoint lines and outgoing stack frames; VS Code and other 1-indexed clients (the default) are unaffected.
